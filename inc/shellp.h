...
// Stream API
//
WINSHELLAPI LPSTREAM WINAPI OpenRegStream(HKEY hkey, LPCWSTR pszSubkey, LPCWSTR pszValue, DWORD grfMode);
...
//
//
WINSHELLAPI void WINAPI PathGetShortPath(LPWSTR pszLongPath);
WINSHELLAPI BOOL WINAPI PathFindOnPath(LPTSTR szFile, LPCTSTR * ppszOtherDirs);
WINSHELLAPI BOOL WINAPI PathYetAnotherMakeUniqueName(LPWSTR  pszUniqueName, LPCWSTR pszPath, LPCWSTR pszShort, LPCWSTR pszFileSpec);
//
...
WINSHELLAPI BOOL WINAPI Win32DeleteFile(LPCWSTR lpszPath);
...
WINSHELLAPI LONG WINAPI PathProcessCommand( LPCWSTR lpSrc, LPWSTR lpDest, int iMax, DWORD dwFlags );
...
WINSHELLAPI STDAPI SHCoCreateInstance(LPCWSTR pszCLSID, const CLSID * lpclsid,
        LPUNKNOWN pUnkOuter, REFIID riid, LPVOID * ppv);
WINSHELLAPI BOOL  WINAPI SignalFileOpen(LPCITEMIDLIST pidl);
WINSHELLAPI LPITEMIDLIST WINAPI SHSimpleIDListFromPath(LPCWSTR pszPath);
...
WINSHELLAPI int   WINAPI Shell_GetCachedImageIndex(LPCWSTR pszIconPath, int iIconIndex, UINT uIconFlags);
...
#ifndef E_PENDING
// BUGBUG (scotth): this is here because 1234 of NT doesn't have this.
...
WINSHELLAPI BOOL WINAPI SHValidateUNC(HWND hwndOwner, LPWSTR pszFile, UINT fConnect);
...
//
// extern HANDLE  WINAPI PifMgr_OpenProperties(LPCTSTR lpszApp, LPCTSTR lpszPIF, UINT hInf, UINT flOpt);
...
// extern HANDLE  WINAPI PifMgr_CloseProperties(int hProps, UINT flOpt);
