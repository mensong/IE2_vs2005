		...
		case VIEWER_JFIF:
			pvi->fDCached = ( gPrefs.bEnableDiskCache
							 && FUpdateBuiltinDCache(
										pvi->format == VIEWER_GIF ? WWW_GIF : WWW_JPEG,
										szURL,
										&pvi->fsOrig,
										dctExpires,
...
void Viewer_SaveAsBitmap_IMG_INLINE(char *tempFile, PCImageInfo pImg, struct Mwin *tw)
{
	BITMAPFILEHEADER bf;
	int adjustedwidth;
	FILE *fp;
...
void Viewer_SaveAsOriginal2(char *tempFile, struct Mwin *tw, char *pCachePath)
{
	FILE *fpRead = NULL;
	FILE *fpWrite = NULL;
	char *pMem;
                        ...
                        PSTR szPath, UINT ucPathBufLen, DWORD dwFlags)
{
    ...
    */
	...
	if (strstr(tempFile, ".BMP"))
		Viewer_SaveAsBitmap_IMG_INLINE(tempFile, pImg, tw);
	else if ( 
			 pPath && 
			  (   bIsAVI ||
		...
		Viewer_SaveAsOriginal2(tempFile, tw, pPath);
	else
	{
		ERR_ReportError(tw, errCantSaveFile, tempFile, "");
		goto exitPoint;
