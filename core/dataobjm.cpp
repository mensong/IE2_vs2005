...
PRIVATE_DATA char s_szFileDescriptorCF[]  = "FileGroupDescriptor"; // CFSTR_FILEDESCRIPTOR;
PRIVATE_DATA char s_szFileContentsCF[]    = "FileContents"; // CFSTR_FILECONTENTS;
...
{
   g_cfURL                 = RegisterClipboardFormatA(s_szURLCF);
   g_cfFileGroupDescriptor = RegisterClipboardFormatA(s_szFileDescriptorCF);
   g_cfFileContents        = RegisterClipboardFormatA(s_szFileContentsCF);
...
PUBLIC_CODE BOOL MakePathReadWrite(PCSTR pcszPath)
{
   ...
   dwOldAttributes = GetFileAttributesA(pcszPath);
   ...
   if (dwOldAttributes != -1)
   {
      if (IS_FLAG_SET(dwOldAttributes, FILE_ATTRIBUTE_READONLY))
      {
         ...
         bResult = SetFileAttributesA(pcszPath, dwOldAttributes ? dwOldAttributes : FILE_ATTRIBUTE_NORMAL);
   ...
   if (hgDropFiles)
   {
      ...
      if (pdf)
      {
         ...
         for (ul = 0; ul < ulcPaths; ul++)
         {
            ...
            lstrcpyA(pszDropPath, rgpcszPaths[ul]);

            TRACE_OUT(("CreateHDrop(): Added %s to HDROP.",
                       pszDropPath));

            // (+ 1) for null terminator.
            pszDropPath += lstrlenA(pszDropPath) + 1;
         }
      ...
      if (piurl->SetURL(pcszURL, 0) == S_OK)
      {
         ...
         if (piurl->QueryInterface(IID_IExtractIcon, (PVOID *)&piei) == S_OK)
         {
                                      ...
                                      &niIcon, &uOutFlags) == S_OK)
            {
               ...
               *phicon = ExtractIconA(wg.hInstance, szIconFile,
                                     niIcon);
