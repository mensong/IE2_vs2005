    ...
    )
{
    ...
    attr = GetFileAttributesA((char *)path);
    if (attr  == 0xffffffff) {
        /* error occured -- map error code and return */
    ...
    )
{
    ...
    number = (unsigned)GetTickCount();

    while (*string)
        string++;

#ifdef _MBCS
//     while ((--string>=template_name) && (!_ismbstrail(template_name,string))
//         && (*string == 'X'))

    while ((--string>=template_name) && (*string == 'X'))
#else
    while (*--string == 'X')
#endif
    {
    ...
    while ((_access(template_name,0) == 0) )
        /* while file exists */
    {
        //errno = 0;
        if (letter == 'z'+1) {
