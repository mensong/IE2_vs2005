	...
	if (NULL != pBufOut){
		...
		if (ssi->dwSSLSystemFlags & WSSA_FLAG_ENCRYPTION_ON){
			if (WSSA_ERROR != SslMakeMac(pBufOut+3, ssi->wciClient1.pKey, 
				...
				myerrno = WSSA_ERROR_DATA_TOO_LARGE;
		}
		else {
			nSizeOut = nSize;
			pBufHead = pBufOut + 16;
		...
		if (WSSA_OK == myerrno) {
			myerrno = send(SS2S(ss),pBufHead, nSizeOut, 0);
			...
			ssi->nSeqSend++;			
			return myerrno - (nSizeOut - nSize);		
		}
	}
	WSASetLastError(myerrno);
	return SOCKET_ERROR;
...
static int SslServerHelloParse(SSI ssi, char *pBuf, int nSize, LPBSAFE_PUB_KEY *ppKey){
	int myerrno, z, x, nCipherSpecSize;

	/*default error as lame error*/
	myerrno = WSSA_ERROR_UNRECOVERABLE;
			...
			if (SSL_CT_X509_CERTIFICATE == ssi->nCertificateType){
				...
				if (ssi->pCertificateData){
					...
					if  (NULL == ssi->pCipherInfo){
						...
						else myerrno = SSL_PE_BAD_CERTIFICATE;
	}}}}}
	WSASetLastError(myerrno);
	return WSSA_ERROR;
}
