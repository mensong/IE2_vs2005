...
int WSSAFNCT WssaReceive(SECURE_SOCKET ss, char FAR * buf, int len, int flags)
{
	...
	if (NULL != ssi)
	{
		...
		{
			/*yes*/
			myerrno = SslHandshakeAsServer(ss);
			if (SOCKET_ERROR == errno) 
				{
				DebugExitINT(WssaReceiveSOCKET_ERROR, myerrno);
				return SOCKET_ERROR;
				}
		}
...
int WSSAFNCT WssaSend(SECURE_SOCKET ss, const char FAR * buf, int len, int flags){
	...
	if (NULL != ssi){
		...
		if (   !(ssi->dwSSLSystemFlags & WSSA_FLAG_HANDSHAKE_DONE)
			...
			/*yes*/
			myerrno = SslHandshakeAsClient(ss);

			if (SOCKET_ERROR == myerrno)
				{
				DebugExitINT(WssaSend, myerrno);
				return SOCKET_ERROR;
				}
...
int WSSAFNCT WssaStartup (WORD wVersionRequired, LPWSADATA lpWSAData)
{
	...
	SeedRandomNumber();
#endif
	myerrno = WSAStartup (wVersionRequired, lpWSAData);
	if (0==myerrno) WssaTableStartup();

	DebugExitINT(WssaStartup, myerrno);
	return myerrno;
}
