...
extern const char g_cszURLLeftDelimiter[]          = EMPTY_STRING;
extern const char g_cszURLRightDelimiter[]         = EMPTY_STRING;
...
PRIVATE_DATA const char s_cszSectionBefore[]       = "[";
PRIVATE_DATA const char s_cszSectionAfter[]        = "]";
PRIVATE_DATA const char s_cszKeyValueSep[]         = "=";
                                             ...
                                             PCSTR pcszFile)
{
   ...
   return(WritePrivateProfileStringA(pcszSection, pcszKey, NULL, pcszFile));
}
...
PRIVATE_CODE HRESULT MassageURL(PSTR pszURL)
{
   ...
   if (! _strnicmp(pszURL, g_cszURLLeftDelimiter,
                   sizeof(g_cszURLLeftDelimiter) - 1))
   {
      // (- 1) for null terminator.

      PSTR pszBase = pszURL + sizeof(g_cszURLLeftDelimiter) - 1;
      PSTR psz;
      PSTR pszEnd;

      // Skip over any "url:" prefix.

      if (! _strnicmp(pszBase, g_cszURLPrefix, g_ucbURLPrefixLen))
         pszBase += g_ucbURLPrefixLen;

      // Find last right delimiter in URL.

      if (*g_cszURLRightDelimiter)
      {
         pszEnd = NULL;

         for (psz = pszBase;
              (psz = strstr(psz, g_cszURLRightDelimiter)) != NULL;
              psz++)
            pszEnd = psz;
      }
      else
         pszEnd = pszBase + lstrlenA(pszBase);

      if (pszEnd)
      {
         // (- 1) for null terminator.

         if (! *(pszEnd + sizeof(g_cszURLRightDelimiter) - 1))
         {
            *pszEnd = '\0';
            lstrcpyA(pszURL, pszBase);
            ...
            WARNING_OUT(("MassageURL(): URL %s does not end with right delimiter %s.",
                         pszURL,
                         g_cszURLRightDelimiter));
      }
      else
         WARNING_OUT(("MassageURL(): No right delimiter %s found in URL %s.",
                      g_cszURLRightDelimiter,
                      pszURL));
   }
   else
      WARNING_OUT(("MassageURL(): URL %s does not start with left delimiter %s.",
                   pszURL,
                   g_cszURLLeftDelimiter));
...
PRIVATE_CODE HRESULT ReadURLFromFile(PCSTR pcszFile, PSTR *ppszURL)
{
   ...
   *ppszURL = NULL;
   ...
   if (pszNewURL)
   {
      ...
      dwcValueLen = GetPrivateProfileStringA(s_cszInternetShortcutSection,
                                            s_cszURLKey, EMPTY_STRING,
      ...
      if (dwcValueLen > 0)
      {
         ...
         if (hr == S_OK)
         {
            ...
            if (ReallocateMemory(pszNewURL, lstrlenA(pszNewURL) + 1,
                                 (PVOID *)&pszShorterURL))
            {
...
PRIVATE_CODE HRESULT WriteURLToFile(PCSTR pcszFile, PCSTR pcszURL)
{
   ...
   if (AnyMeat(pcszURL))
   {
      ...
      ncbLen = sizeof(g_cszURLLeftDelimiter) - 1 +
               lstrlenA(pcszURL) +
               sizeof(g_cszURLRightDelimiter);
      ASSERT(ncbLen > 0);

      pszURLRHS = new(char[ncbLen]);

      if (pszURLRHS)
      {
         lstrcpyA(pszURLRHS, g_cszURLLeftDelimiter);
         lstrcatA(pszURLRHS, pcszURL);
         lstrcatA(pszURLRHS, g_cszURLRightDelimiter);

         ASSERT(lstrlen(pszURLRHS) == ncbLen - 1);

         hr = (WritePrivateProfileStringA(s_cszInternetShortcutSection, s_cszURLKey, pszURLRHS, pcszFile))
              ? S_OK
              : E_FAIL;
      ...
      hr = (DeletePrivateProfileString(s_cszInternetShortcutSection, s_cszURLKey, pcszFile))
           ? S_OK
           : E_FAIL;
   ...
   *pniIcon = 0;
   ...
   if (dwcValueLen > 0)
   {
      ...
      if (dwcValueLen > 0)
      {
         ...
         if (sscanf(rgchNewIconIndex, "%d", &niIcon) == 1)
         {
            // (+ 1) for null terminator.

            *ppszIconFile = new(char[lstrlenA(rgchNewIconFile) + 1]);

            if (*ppszIconFile)
            {
               lstrcpyA(*ppszIconFile, rgchNewIconFile);
               *pniIcon = niIcon;
                                             ...
                                             int niIcon)
{
   ...
   if (AnyMeat(pcszIconFile))
   {
      ...
      ncLen = wsprintfA(rgchIconIndexRHS, "%d", niIcon);
      ASSERT(ncLen > 0);
      ASSERT(ncLen < sizeof(rgchIconIndexRHS));
      ASSERT(ncLen == lstrlen(rgchIconIndexRHS));

      hr = (WritePrivateProfileStringA(s_cszInternetShortcutSection,
                                      s_cszIconFileKey, pcszIconFile,
                                      pcszFile) &&
            WritePrivateProfileStringA(s_cszInternetShortcutSection,
...
PRIVATE_CODE HRESULT ReadHotkeyFromFile(PCSTR pcszFile, PWORD pwHotkey)
{
   ...
   *pwHotkey = 0;
   ...
   if (dwcValueLen > 0)
   {
      ...
      if (sscanf(rgchHotkey, "%u", &uHotkey) == 1)
      {
...
PRIVATE_CODE HRESULT WriteHotkeyToFile(PCSTR pcszFile, WORD wHotkey)
{
   ...
   if (wHotkey)
   {
      ...
      ncLen = wsprintfA(rgchHotkeyRHS, "%u", (UINT)wHotkey);
      ASSERT(ncLen > 0);
                                                  ...
                                                  PSTR *ppszWorkingDirectory)
{
   ...
   *ppszWorkingDirectory = NULL;
   ...
   if (dwcValueLen > 0)
   {
      ...
      if (GetFullPathNameA(rgchDirValue, sizeof(rgchFullPath), rgchFullPath,
                          &pszFileName) > 0)
      {
         // (+ 1) for null terminator.

         *ppszWorkingDirectory = new(char[lstrlenA(rgchFullPath) + 1]);

         if (*ppszWorkingDirectory)
         {
            lstrcpyA(*ppszWorkingDirectory, rgchFullPath);
...
PRIVATE_CODE HRESULT ReadShowCmdFromFile(PCSTR pcszFile, PINT pnShowCmd)
{
   ...
   *pnShowCmd = g_nDefaultShowCmd;
   ...
   if (dwcValueLen > 0)
   {
      ...
      if (sscanf(rgchNewShowCmd, "%d", &nShowCmd) == 1)
      {
         *pnShowCmd = nShowCmd;
...
PRIVATE_CODE HRESULT WriteShowCmdToFile(PCSTR pcszFile, int nShowCmd)
{
   ...
   if (nShowCmd != g_nDefaultShowCmd)
   {
      ...
      ncLen = wsprintfA(rgchShowCmdRHS, "%d", nShowCmd);
      ASSERT(ncLen > 0);
                                                       ...
                                                       UINT ucbLen)
{
   ...
   if (m_pszFile)
   {
      MyLStrCpyN(pszFile, m_pszFile, ucbLen);
...
HRESULT STDMETHODCALLTYPE InternetShortcut::GetCurFile(LPOLESTR *ppwszFile)
{
   ...
   if (SUCCEEDED(hr))
   {
      ...
      // (+ 1) for null terminator.
      *ppwszFile = (LPOLESTR)/*SHAlloc*/malloc((lstrlenW(pwszTempFile) + 1) *
                                     sizeof(*pwszTempFile));
...
DWORD STDMETHODCALLTYPE InternetShortcut::GetFileContentsSize(void)
{
   ...
   dwcbLen = sizeof(s_cszSectionBefore) - 1 +
             sizeof(s_cszInternetShortcutSection) - 1 +
                                                            ...
                                                            PSTGMEDIUM pstgmed)
{
   ...
   if (IS_FLAG_SET(pfmtetc->tymed, TYMED_HGLOBAL))
   {
      if (m_pszURL)
      {
         ...
         // (+ 1) for null terminator.
         hgURL = GlobalAlloc(0, lstrlenA(m_pszURL) + 1);
                                                            ...
                                                            PSTGMEDIUM pstgmed)
{
   ...
   if (IS_FLAG_SET(pfmtetc->tymed, TYMED_HGLOBAL))
   {
      ...
      if (hgFileGroupDesc)
      {
         LPFILEGROUPDESCRIPTORA pfgd;
         
         pfgd = (LPFILEGROUPDESCRIPTORA)GlobalLock(hgFileGroupDesc);
         ...
         if (EVAL(pfgd))
         {
            ...
            if (m_pszFile)
            {
               MyLStrCpyN(pfd->cFileName, ExtractFileName(m_pszFile),
            ...
            {
               if (EVAL(LoadStringA(GetThisModulesHandle(),
   ...
   if (IS_FLAG_SET(pfmtetc->tymed, TYMED_HGLOBAL))
   {
      ...
      if (hgFileContents)
      {
         ...
         if (EVAL(pszFileContents))
         {
            ...
            CopyMemory(psz, s_cszInternetShortcutSection, sizeof(s_cszInternetShortcutSection) - 1);
            psz += sizeof(s_cszInternetShortcutSection) - 1;
            ...
            if (m_pszURL)
            {
               ...
               CopyMemory(psz, s_cszURLKey, sizeof(s_cszURLKey) - 1);
               psz += sizeof(s_cszURLKey) - 1;

               CopyMemory(psz, s_cszKeyValueSep, sizeof(s_cszKeyValueSep) - 1);
               psz += sizeof(s_cszKeyValueSep) - 1;

               CopyMemory(psz, m_pszURL, lstrlenA(m_pszURL));
               psz += lstrlenA(m_pszURL);
            }
