...
#define REGSTR_PATH_INTERNET_SETTINGS_A "Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings"
...
static const CHAR szRegPathInternetSettings[] = REGSTR_PATH_INTERNET_SETTINGS_A;
...
static const CHAR szRegPathRNAService[] = /*REGSTR_PATH_SERVICES*/ "System\\CurrentControlSet\\Services\\RemoteAccess";
static const CHAR szRegPathTCP[] = /*REGSTR_PATH_VXD*/ "System\\CurrentControlSet\\Services\\VxD\\MSTCP";
...
static const CHAR szRegPathComputerName[] = "System\\CurrentControlSet\\Control\\ComputerName\\ComputerName"; /*REGSTR_PATH_COMPUTRNAME*/
static const CHAR szRegValComputerName[] = "ComputerName"; /*REGSTR_VAL_COMPUTRNAME*/
#pragma data_seg()
...
VOID AutodialHookCallback(DWORD dwOpCode,LPCVOID lpParam)
{
	...
	if (!fProcessedRecently) {
		...
		HWND hwndMonitorApp = FindWindowA(szAutodialMonitorClass,NULL);
		if (hwndMonitorApp) {
			...
			if (lpParam) {
				...
				if (pszLocalHostname) {
					...
					if (GetRegKeyValue(HKEY_LOCAL_MACHINE,szRegPathTCP,
						...
						ERROR_SUCCESS) {
						if (!lstrcmpiA(pszLocalHostname,(LPSTR) lpParam))
							fDontProcess = TRUE;
					}
...
BOOL WINAPI InetEnsureConnected(HWND hwndParent,DWORD dwFlags)
{
	...
	if (RegOpenKeyA(HKEY_CURRENT_USER,szRegPathInternetSettings,&hKey) == ERROR_SUCCESS) {
		DWORD dwData,dwSize;

		dwSize = sizeof(dwData);
        if (RegQueryValueExA(hKey,szRegValEnableAutodial,NULL,NULL,
		...
		if (fAutodialEnabled) {
			...
			dwSize = sizeof(dwData);
	        if (RegQueryValueExA(hKey,szRegValEnableAutoDisconnect,NULL,NULL,
	        	(LPBYTE) &dwData,&dwSize) == ERROR_SUCCESS && dwData) {
				fAutoDisconnectEnabled = TRUE;
			}

			dwSize = sizeof(dwData);
	        if (RegQueryValueExA(hKey,szRegValEnableSecurityCheck,NULL,NULL,
	        	(LPBYTE) &dwData,&dwSize) == ERROR_SUCCESS && dwData) {
	...
	if ((RegQueryValueExA(hKeyRNA,szRegValRemoteConnection,NULL,NULL,
    ...
    if (RegOpenKeyA(HKEY_CURRENT_USER,szRegPathRemoteAccess,&hKey) == ERROR_SUCCESS) {
     	if ((RegQueryValueExA(hKey,szRegValInternetEntry,NULL,NULL,
        	(LPBYTE) szEntryName,&dwSize) == ERROR_SUCCESS) && lstrlenA(szEntryName)) {
        	fHaveEntryName = TRUE;
		}
...
INTSHCUTAPI BOOL WINAPI InetIsOffline(DWORD dwFlags)
{
	...
	if (fUserCancelled) {
		...
		// check the registry to determine RNA state.
	    DWORD dwVal=0,dwSize=sizeof(dwVal);
		if (hKeyRNA && (RegQueryValueExA(hKeyRNA,szRegValRemoteConnection,NULL,
	...
	pdwRet)
{
	...
	*pdwRet = ERROR_SUCCESS;
	...
	HKEY hkeyProfile;
	if (RegOpenKeyA(HKEY_CURRENT_USER,szRegPathRNAProfile,&hkeyProfile) ==
		ERROR_SUCCESS) {
		HKEY hkeyEntry;
		if (RegOpenKeyA(hkeyProfile,pszEntryName,&hkeyEntry) == ERROR_SUCCESS) {
			...
			if ((RegQueryValueExA(hkeyEntry,szRegValAutodialDllName,NULL,NULL,
				(LPBYTE) szDllName,&cbDllName) == ERROR_SUCCESS) &&
				(RegQueryValueExA(hkeyEntry,szRegValAutodialFcnName,NULL,NULL,
				...
				HINSTANCE hinstDialerDll = LoadLibraryA(szDllName);
				if (hinstDialerDll) {
...
BOOL InitAutodialModule(void)
{
    ...
    UINT uErr = RegCreateKeyA(HKEY_CURRENT_USER,szRegPathInternetSettings,&hKeyTmp);
    if (uErr == ERROR_SUCCESS) {
    	DWORD dwVal,dwSize;

		// is autodial enabled?
        dwSize = sizeof(dwVal);
        if (RegQueryValueExA(hKeyTmp,szRegValEnableAutodial,NULL,NULL,(LPBYTE)
	...
	if (fAutodialEnabled) {
	    ...
	    uErr=RegCreateKeyA(HKEY_LOCAL_MACHINE,szRegPathRNAService,&hKeyRNA);
...
BOOL LoadRNADll(VOID)
{
    ...
    if (!LoadStringA(GetThisModulesHandle(),IDS_RNADLL_FILENAME,
    	szDllFilename,sizeof(szDllFilename)))
        return FALSE;

	// load the DLL
	hInstRNADll = LoadLibraryA(szDllFilename);
    if (!hInstRNADll)
    	return FALSE;
...
BOOL CALLBACK MyEnumWindowsProc(HWND hwnd, LPARAM lparam)
{
	...
	if(!IsWindowVisible(hwnd))
		return TRUE;
	if(GetClassNameA(hwnd, szTemp, SMALLBUFLEN)==0)
		return TRUE; // continue enumerating
	if(strcmp(szTemp, szRnaAppWindowClass)!=0)
		return TRUE;
	if(GetWindowTextA(hwnd, szTemp, SMALLBUFLEN)==0)
		return TRUE;
	szTemp[SMALLBUFLEN] = 0;
	uLen1 = lstrlenA(szTemp);
	if (uLen1 > 5)
		uLen1 -= 5; // skip last 5 chars of title (avoid "...")
	pszTitle = (PSTR)lparam;
	ASSERT(pszTitle);
	uLen2 = lstrlenA(pszTitle);
...
VOID MinimizeRNAWindow(CHAR * pszConnectoidName)
{
	...
	LoadStringA(GetThisModulesHandle(), IDS_CONNECTED_TO, pFmt, SMALLBUFLEN);
	// build the title
	wsprintfA(pTitle, pFmt, pszConnectoidName);
...
BOOL StartAutodisconnectMonitoring(CHAR * pszConnectoidName)
{
	...
	if (hwndMonitorApp) {
		...
		if (LoadStringA(GetThisModulesHandle(),IDS_DIALMON_FILENAME,
			...
			wsprintfA(szCommandLine,"%s %s",szDialmonFilename,pszConnectoidName);

			// launch disconnect monitoring app as a process
			fSuccess = CreateProcessA(NULL,szCommandLine,
...
BOOL PerformSecurityCheck(BOOL * pfNeedRestart)
{
	...
	*pfNeedRestart = FALSE;
	...
	LoadStringA(GetThisModulesHandle(),IDS_INETCFG_FILENAME,szFilename,
		sizeof(szFilename));

	// load the inetcfg dll
	hinstInetWiz = LoadLibraryA(szFilename);
	ASSERT(hinstInetWiz);
	if (hinstInetWiz) {
...
BOOL ChooseConnectoidDlgInit(HWND hDlg)
{
	...
	if (RegOpenKeyA(HKEY_CURRENT_USER,szRegPathRemoteAccess,&hKey)
		== ERROR_SUCCESS) {
		CHAR szEntryName[RAS_MaxEntryName + 1]="";
		DWORD dwSize=sizeof(szEntryName);
		if ((RegQueryValueExA(hKey,szRegValDefaultEntry,NULL,NULL,
			(LPBYTE) szEntryName,&dwSize) == ERROR_SUCCESS) &&
			lstrlenA(szEntryName)) {
...
BOOL ChooseConnectoidDlgOK(HWND hDlg,BOOL * pfRetCode)
{
	...
	if (IsDlgButtonChecked(hDlg,IDD_DISABLEAUTODIAL)) {
        ...
        if (RegCreateKeyA(HKEY_CURRENT_USER,szRegPathInternetSettings,&hKey)
        	== ERROR_SUCCESS) {
			DWORD dwVal = 0;
    		RegSetValueExA(hKey,szRegValEnableAutodial,0,REG_BINARY,
		...
		*pfRetCode = FALSE;	// return FALSE to caller
        ...
        if (GetWindowTextA(hwndCB,szEntryName,sizeof(szEntryName))) {

	        if (RegCreateKeyA(HKEY_CURRENT_USER,szRegPathRemoteAccess,&hKey)
	        	== ERROR_SUCCESS) {
	    		RegSetValueExA(hKey,szRegValInternetEntry,0,REG_SZ,(LPBYTE) szEntryName,
	        	    lstrlenA(szEntryName)+1);
	            RegCloseKey(hKey);
	        }
			...
			if (pszReturnBuf) {
        		lstrcpyA(pszReturnBuf,szEntryName);
			}
...
BOOL EditConnectoid(HWND hDlg)
{
    ...
    GetWindowTextA(hwndCombo,szEntryName,sizeof(szEntryName));

	if (lstrlenA(szEntryName)) {
