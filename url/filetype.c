...
// and expand REG_STRING_SZ variables.  Also we must cope with late binding
// to the SHRegQueryValue functions which may not be available on Win95.
...
typedef LONG (*P_SHRegQueryValue)( HKEY hKey, LPCTSTR lpSubKey, LPTSTR lpValue, PLONG lpcbValue );

P_SHRegQueryValue lpSHRegQueryValue = NULL;
LONG UrlRegQueryValue( HKEY hKey, LPCTSTR lpSubKey, LPTSTR lpValue, PLONG lpcbValue );

#define RegQueryValue(hKey, lpSubKey, lpValue, lpcbValue) UrlRegQueryValue(hKey, lpSubKey, lpValue, lpcbValue )

#else   // X86

#ifdef UNICODE
#define RegQueryValue(hKey, lpSubKey, lpValue, lpcbValue) SHRegQueryValueW(hKey, lpSubKey, lpValue, lpcbValue )
#else
#define RegQueryValue(hKey, lpSubKey, lpValue, lpcbValue) SHRegQueryValueA(hKey, lpSubKey, lpValue, lpcbValue )
#endif
...
#if defined(WINNT) && (defined(_X86_) || defined(DAYTONA_BUILD))
#ifdef UNICODE
        lpSHRegQueryValue = (P_SHRegQueryValue)GetProcAddress(hShellDll, (LPCSTR)510 ); 
#else
        lpSHRegQueryValue = (P_SHRegQueryValue)GetProcAddress(hShellDll, (LPCSTR)508 ); 
#endif // UNICODE
#endif // WINNT && _X86_
    }
...
VOID FT_OnCommand(PFILETYPESDIALOGINFO pFTDInfo, HWND hDialog, WPARAM wParam, LPARAM lParam)
{
	...
	switch(idCmd)
	{
		...
		case IDC_FT_PROP_EDIT:
			...
			DialogBoxParam(hinstCabinet, MAKEINTRESOURCE(DLG_FILETYPEOPTIONSEDIT), hDialog, FTEdit_DlgProc, (LPARAM)pFTDInfo);
			DisplayOpensWithObjects(pFTDInfo, hDialog);
                        DisplayDocObjects(pFTDInfo, hDialog);
			break;
		...
		case IDC_FT_PROP_REMOVE:
			...
			if(ShellMessageBox(hinstCabinet, hDialog, 
							MAKEINTRESOURCE(IDS_FT_MB_REMOVETYPE), 
							MAKEINTRESOURCE(IDS_FT), 
	...
	switch(message)
	{
...
#if defined(WINNT) && defined(_X86_)
            lpSHRegQueryValue = NULL;
#endif
		...
		case WM_HELP:
			WinHelp((HWND)((LPHELPINFO) lParam)->hItemHandle,
                    ...
                    NULL,
#endif   /* MIME */
    				HELP_WM_HELP, (DWORD)(LPSTR)aFileTypeOptionsHelpIDs);
			return TRUE;
...
#pragma data_seg(DATASEG_READONLY)
static const char c_szFileViewer[] = "QuickView";
static const char c_szDefViewerKeyName[] = "QuickView\\*";
static const char c_szDefViewer[] = "*";
#pragma data_seg()

BOOL FTEdit_ISViewable (PFILETYPESDIALOGINFO pFTDInfo)
{
    char szViewers[MAX_PATH+40];
    LONG cbValue;
    ...
    if(pFTDInfo->pFTInfo->hDPAExt != (HDPA)NULL)
    {
        ...
        for (iExt = 0; iExt < cExt; iExt++)
        {
            ...
            lstrcat(szViewers, "\\");
	    lstrcat(szViewers, (LPSTR)DPA_FastGetPtr(
...
#ifdef FT_DEBUG
	        DebugMsg(DM_TRACE, "FTEDIT_IsViewable: by Ext %s", szViewers);
#endif
		RegCloseKey(hkeyExt);
                return(TRUE);
	    }
...
void FTEdit_SetViewable (PFILETYPESDIALOGINFO pFTDInfo, BOOL fViewable)
{
    ...
    else
    {
        ...
        lRegDeleteKeyA (pFTDInfo->pFTInfo->hkeyFT, c_szFileViewer);
        ...
        for (iExt = 0; iExt < cExt; iExt++)
        {
            ...
            lstrcat(szViewers, (LPSTR)DPA_FastGetPtr(
                    pFTDInfo->pFTInfo->hDPAExt, iExt));
            lRet = lRegDeleteKeyA(HKEY_CLASSES_ROOT, szViewers);

#ifdef FT_DEBUG
            DebugMsg(DM_TRACE, "FTEDIT_SetViewable: Delete key %s ret=%x",
                    szViewers, lRet);
#endif
        }
    }
...
BOOL FTEdit_IsExtShowable(PFILETYPESDIALOGINFO pFTDInfo)
{
	...
	return(RegQueryValueEx(pFTDInfo->pFTInfo->hkeyFT, (LPSTR)c_szShowExt, NULL, &dwType, (LPSTR)szShowExt, &dwShowExt)
		== ERROR_SUCCESS);
}
...
BOOL FTEdit_AreDefaultViewersInstalled()
{
    ...
    {
        if (RegEnumKey(hkey, 0, szValue, sizeof(szValue)) == ERROR_SUCCESS)
            fRet = TRUE;
        RegCloseKey(hkey);
    }
...
BOOL FTEdit_OnInitDialog(HWND hDialog, WPARAM wParam, LPARAM lParam)
{
	...
	case IDC_FT_PROP_NEW:
		...
		LoadString(hinstCabinet, IDS_ADDNEWFILETYPE, szTitle, ARRAYSIZE(szTitle));
...
VOID ChangeDefaultButtonText(HWND hDialog, DWORD dwIDS)
{
	...
	if(LoadString(hinstCabinet, dwIDS, szStr, sizeof(szStr)))
...
VOID FTEdit_OnRemove(PFILETYPESDIALOGINFO pFTDInfo, HWND hDialog)
{
	...
	if(ShellMessageBox(hinstCabinet, hDialog, 
		...
		MAKEINTRESOURCE(IDS_FT), MB_YESNO | MB_ICONQUESTION) == IDNO)
		return;
	...
	OkToClose_NoCancel(hinstCabinet, hDialog);
...
VOID FTEdit_OnChangeIcon(PFILETYPESDIALOGINFO pFTDInfo, HWND hDialog)
{
    ...
    if( *sfi.szDisplayName )
    {
        ...
        if ( UrlPathProcessCommand( sfi.szDisplayName, sfi.szDisplayName, SIZEOF(sfi.szDisplayName), PPCF_NODIRECTORIES ) == -1 )
        {
            ...
            lPathRemoveBlanksA( sfi.szDisplayName );
            lPathUnquoteSpacesA( sfi.szDisplayName );
        }
        ...
        {
            if(lPathIsRelativeA(sfi.szDisplayName))
                lPathFindOnPathA(sfi.szDisplayName, NULL);        // search for exe
        }
    }
...
VOID FTEdit_OnCommand(PFILETYPESDIALOGINFO pFTDInfo, HWND hDialog, WPARAM wParam, LPARAM lParam)
{
	...
	switch(idCmd)
	{
		...
		case IDC_FT_EDIT_NEW:
		{
			...
			if(DialogBoxParam(hinstCabinet, MAKEINTRESOURCE(DLG_FILETYPEOPTIONSCMD), hDialog, FTCmd_DlgProc, (LPARAM)pFTDInfo))
			{
				OkToClose_NoCancel(hinstCabinet, hDialog);
				PropSheet_CancelToClose(GetParent(pFTDInfo->hPropDialog));
			}
	...
	switch(message)
	{
		...
		case WM_HELP:
			WinHelp((HWND)((LPHELPINFO) lParam)->hItemHandle,
                    ...
                    NULL,
#endif   /* MIME */
     				HELP_WM_HELP, (DWORD)(LPSTR)aEditFileTypesHelpIDs);
			return TRUE;
...
VOID FTCmd_OnInitDialog(HWND hDialog, WPARAM wParam, LPARAM lParam)
{
	...
	if(pFTDInfo->dwCommandEdit == IDC_FT_EDIT_EDIT)
	{
		...
		if(LoadString(hinstCabinet, IDS_FT_EDITTITLE, szBuf, sizeof(szBuf)))
		{
...
LONG DeleteDDEKeys(LPCSTR pszKey)
{
	...
	return(lRegDeleteKeyA(HKEY_CLASSES_ROOT, szBuf));
}
		...
		if(lstrcmpi(szPath, pFTDInfo->pFTCInfo->szCommand) != 0)
		{
			...
			if(IsDefaultAction(pFTDInfo, pFTDInfo->pFTCInfo->szActionKey))
			{
				   ...
				   (pFTDInfo->dwCommand == IDC_FT_PROP_NEW))
				{
					...
					iImageIndex = lShell_GetCachedImageIndexA(szPath, 0, GIL_SIMULATEDOC);
...
//#ifdef _X86_
	...
	else
	{
		...
		LoadStringA(hinstCabinet, IDS_OPENAS, szTitle, sizeof(szTitle));
		LoadStringA(hinstCabinet, IDS_EXE, szExe, sizeof(szExe));
		LoadStringA(hinstCabinet, IDS_PROGRAMSFILTER, szFilters, sizeof(szFilters));
		psz = szFilters;
        while (*psz)
        {
            if (*psz == (char)('#'))
	...
	szPath[0] = 0;
	LoadString(hinstCabinet, IDS_OPENAS, szTitle, sizeof(szTitle));
	if (GetFileNameFromBrowse(hDialog, szPath, sizeof(szPath), NULL, 
	...
	switch(message)
	{
		...
		case WM_HELP:
			WinHelp((HWND)((LPHELPINFO) lParam)->hItemHandle, NULL,
				HELP_WM_HELP, (DWORD)(LPSTR) aEditCommandHelpIDs);
			return TRUE;
...
VOID OkToClose_NoCancel(HINSTANCE hinst, HWND hDialog)
{
	...
	if(LoadString(hinst, IDS_FT_CLOSE, szStr1, sizeof(szStr1)))
	{
...
HICON GetDocIcon(PFILETYPESDIALOGINFO pFTDInfo, LPSTR lpszStr)
{
	...
	if(pFTDInfo->pFTInfo->dwAttributes & FTA_HasExtension)
	{
		...
		else
		{
			...
			iImageIndex = lShell_GetCachedImageIndexA(lpszStr, 0, GIL_SIMULATEDOC);
			hIcon = ImageList_ExtractIcon(hinstCabinet, s_himlSysLarge, iImageIndex);
		}
	}
...
VOID DisplayOpensWithObjects(PFILETYPESDIALOGINFO pFTDInfo, HWND hDialog)
{
	...
	if(*szFile)
	{
                ...
                szFile, cbT, c_szExefileOpenCommand, cbT) == 2)
		{
			...
			LoadString(hinstCabinet, IDS_FT_EXEFILE, szFullPath, sizeof(szFullPath));
		}
		else
		{
            if ( UrlPathProcessCommand( szFile, szFile, MAX_PATH, PPCF_NODIRECTORIES ) == -1 )
            {
                ...
                lPathRemoveBlanksA(szFile);
                lPathUnquoteSpacesA(szFile);
                
                if (lPathIsRelativeA(szFile))
                  lPathFindOnPathA(szFile, NULL);
            }
		...
		// Display exe name
		lstrcpy(szFile, lPathFindFileNameA(szFullPath));	// Strip off the path
		if(*szFile)
		{
    ...
    if(iLength != 0 && iLength < MAXEXTSIZE)
	{
		...
		*szId = '\0';
		if((RegQueryValue(HKEY_CLASSES_ROOT, szExt, szId, &dwId) == ERROR_SUCCESS) && *szId)
		{
			...
			*szBuf = '\0';
			*szStr2 = '\0';
			if(LoadString(hinstCabinet, IDS_FT_MB_EXTTEXT, szStr1, sizeof(szStr1)))
			{
				if(LoadString(hinstCabinet, IDS_FT, szStr2, sizeof(szStr2)))
				{
...
#pragma data_seg()
...
#ifndef WINNT // another bobday fix
			if(pFTDInfo->pFTInfo->hkeyFT)
			{
				RegCloseKey(pFTDInfo->pFTInfo->hkeyFT);
#ifdef FT_DEBUG
				DebugMsg(DM_ERROR, "IDC_FT_EDIT_EXT: Closing non-null hkey");
#endif
			}
#endif // WINNT
		}
	...
	else
	{
		...
		*szBuf = '\0';
		*szStr2 = '\0';
		if(LoadString(hinstCabinet, IDS_FT_MB_NOEXT, szStr1, sizeof(szStr1)))
		{
			if(LoadString(hinstCabinet, IDS_FT, szStr2, sizeof(szStr2)))
			{
				if(lstrlen(szStr1) + lstrlen(szExt) + lstrlen(szId) < sizeof(szBuf))
...
BOOL ActionExeIsValid(HWND hDialog, BOOL bMBoxFlag)
{
	...
	PathRemoveArgs(szPath);
    lPathUnquoteSpacesA(szPath);
	lstrcpy(szFileName, lPathFindFileNameA(szPath));

	if(!(*szPath) || (!(lPathIsExeA(szPath))) || 
		((!(lPathFileExistsA(szPath))) && (!(lPathFindOnPathA(szFileName, NULL)))))
	{
		if(bMBoxFlag)
		{
			// Tell user that this exe is invalid
...
int FTEdit_InitListView(PFILETYPESDIALOGINFO pFTDInfo)
{
	...
	iSubKey = 0;
	if(RegOpenKeyEx(pFTDInfo->pFTInfo->hkeyFT, c_szShell, 
			(DWORD)NULL, 
#ifdef WINNT  // another bobday fix...
...
VOID AddExtDot(LPSTR pszExt, UINT iExt)
{
    ...
    lPathRemoveBlanksA(pszExt);       // remove 1st and last blank
...
DWORD SetVerbAttributes(HKEY hkeyFT, LPSTR pszVerb, DWORD dwAttributes)
{
	...
	if((hkeyFT != NULL) && 
		(RegOpenKeyEx(hkeyFT, szVerbKey, (DWORD)NULL, KEY_SET_VALUE, &hk) == ERROR_SUCCESS))
	{
		dwSize = sizeof(dwAttributes);
		err = RegSetValueEx(hk, (LPTSTR)c_szEditFlags, 0, REG_BINARY, 
...
DWORD GetVerbAttributes(HKEY hkeyFT, LPSTR pszVerb)
{
	...
	wsprintf(szVerbKey, c_szTemplateSS, c_szShell, pszVerb);
	if((hkeyFT != NULL) && 
		(RegOpenKeyEx(hkeyFT, szVerbKey, (DWORD)NULL, 
					KEY_QUERY_VALUE, &hk) == ERROR_SUCCESS))
	{
		dwSize = sizeof(dwAttributes);
                err = RegQueryValueEx(hk, (LPTSTR)c_szEditFlags, NULL, 
							&dwType, (LPBYTE)&dwAttributes, &dwSize);
                if (err != ERROR_SUCCESS ||
#ifdef WINNT
                    dwType != REG_BINARY ||
#else
...
VOID FT_MergeDuplicates(HWND hwndLV)
{
	...
	for(i = 1; i < iCnt; i++)
	{
		...
		if(lstrcmpi(pFTInfo1->szId, pFTInfo2->szId) == 0)	// we have a match
		{
            ...
            __asm int 3;
			DPA_InsertPtr(pFTInfo1->hDPAExt, 0x7fff, 
					(LPVOID)DPA_FastGetPtr(pFTInfo2->hDPAExt,0));
...
HICON GetDefaultIcon(HKEY *hkeyFT, LPSTR pszId, DWORD dwIconType)
{
	...
	if(*hkeyFT != (HKEY)NULL)
	{
		...
		if (err == ERROR_SUCCESS && *szDefaultIcon)
		{
			...
			iImage = lShell_GetCachedImageIndexA(szDefaultIcon, iIconIndex, 0);
...
#else
	if(RegOpenKeyEx(hk, pszKey, (DWORD)NULL, KEY_QUERY_VALUE, &hk1) == ERROR_SUCCESS)
#endif
	{
		if(lRegDeleteKeyA(hk1, szAction) != ERROR_SUCCESS)
			lRC = !ERROR_SUCCESS;
		RegCloseKey(hk1);
	}
...
LONG RemoveFileType(PFILETYPESDIALOGINFO pFTDInfo)
{
	...
	if(*pFTDInfo->pFTInfo->szId)
		if(lRegDeleteKeyA(HKEY_CLASSES_ROOT, pFTDInfo->pFTInfo->szId) != ERROR_SUCCESS)
		    	lRC = !ERROR_SUCCESS;
...
#pragma data_seg()
						...
						{
							if(lRegDeleteKeyA(HKEY_CLASSES_ROOT, pszExt) != ERROR_SUCCESS)
		        				lRC = !ERROR_SUCCESS;
                                                }
					}
				}
...
Error:
		...
		AnsiUpper(szExt);
		LoadString(hinstCabinet, IDS_EXTTYPETEMPLATE, szTemplate, ARRAYSIZE(szTemplate));
...
{
    LPSTR pExt = lPathFindExtensionA(pszPath);
    if (*pExt)
    {
        Assert(*pExt == '.');
        *pExt = 0;    // null out the "."
...
int ParseIconLocation(LPSTR pszIconFile)
{
	...
	if (pszComma) {
		*pszComma++ = 0;	    // terminate the icon file name.
		...
		iIndex = /*StrToInt*/ atoi(pszComma);
	}
	lPathRemoveBlanksA(pszIconFile);
	return iIndex;
}
...
///	{
		...
		lPathRemoveBlanksA(szFile);
		if(lPathIsRelativeA(szFile))
			lPathFindOnPathA(szFile, NULL);	// search for exe
...
#if (defined(_X86_) && defined(WINNT)) || !defined(WINNT)
...
LONG UrlPathProcessCommand2( LPCTSTR lpSrc, LPTSTR lpDest, int iDestMax, DWORD dwFlags )
{
    ...
    if ( lpSrc )
    {
        ...
        if ( *lpSrc == TEXT('\"') )
        {
            ...
            if ( ((dwFlags & PPCF_FORCEQUALIFY) || lPathIsRelativeA( szName ))
        ...
        else
        {
            ...
            bQualify = lPathIsRelativeA( lpSrc ) || ((dwFlags & PPCF_FORCEQUALIFY) != 0);
...
// Query the registry for a value and generate an expanded version if required.

#if defined(WINNT) && ( defined(_X86_) || defined(DAYTONA_BUILD))

LONG UrlRegQueryValue( HKEY hKey, LPCTSTR lpSubKey, LPTSTR lpValue, PLONG lpcbValue )
{
    if ( RUNNING_NT && lpSHRegQueryValue )
        return lpSHRegQueryValue( hKey, lpSubKey, lpValue, lpcbValue );
    else
#ifdef UNICODE
