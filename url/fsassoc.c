...
extern LPSTR WINAPI lPathFindFileNameA(LPCSTR);
extern LPSTR WINAPI lPathFindExtensionA(LPCSTR);
extern LPSTR WINAPI lPathGetArgsA(LPCSTR);
extern void  WINAPI lPathUnquoteSpacesA(LPSTR);
extern void  WINAPI lPathQuoteSpacesA(LPSTR);
extern BOOL  WINAPI lPathIsRelativeA(LPCSTR);
extern BOOL  WINAPI lPathFindOnPathA(LPSTR pszFile, LPCSTR *ppszOtherDirs);
extern BOOL  WINAPI lPathIsExeA(LPCSTR);
extern BOOL  WINAPI lPathFileExistsA(LPCSTR);
extern void  WINAPI lPathRemoveBlanksA(LPTSTR);
extern int   WINAPI lShell_GetCachedImageIndexA(LPCSTR, int, UINT);
extern BOOL  WINAPI lPathRemoveFileSpecA(LPSTR);
extern BOOL  WINAPI lPathYetAnotherMakeUniqueNameA(LPSTR,LPCSTR,LPCSTR,LPCSTR);
extern LONG  WINAPI lRegDeleteKeyA(HKEY, LPCSTR);
...
void GetCmdLine(LPCSTR szKey, LPSTR szValue, LONG cbValue)
{
    ...
    RegQueryValueExA(HKEY_CLASSES_ROOT, szTemp,  NULL, &dwType, szValue, &cbValue);
}
...
void FillListWithClasses(HWND hwnd, BOOL fComboBox, UINT uFlags)
{
	...
	for (i = 0; RegEnumKey(HKEY_CLASSES_ROOT, i, szClass, sizeof(szClass)) == ERROR_SUCCESS; i++)
	{
	    ...
	    if ((RegQueryValueExA(HKEY_CLASSES_ROOT, szExt,  NULL, &dwType, szClass, &lcb) != ERROR_SUCCESS) || (lcb == 0))
...
BOOL GetClassDescription(HKEY hkClasses, LPCSTR pszClass, LPSTR szDisplayName, int cbDisplayName, UINT uFlags)
{
    ...
    if (uFlags & GCD_ALLOWPSUDEOCLASSES)
    {
	    ...
	    if ((RegQueryValueExA(hkClasses, pszClass, NULL, &dwType, szClass, &lcb) != ERROR_SUCCESS) || (lcb == 0))
	    {
    ...
    if (RegQueryValueExA(hkClasses, pszClass, NULL, &dwType, szDisplayName, &lcb) != ERROR_SUCCESS || (lcb < 2))
    return FALSE;
...
void FillListWithApps(HWND hwndList)
{
	...
	if (RegQueryValueExA(HKEY_CLASSES_ROOT, szKey, NULL, &dwType, szValue, &lTmp) == ERROR_SUCCESS)
	{
	...
	//
		...
		if (pai)
		{
			...
			if (RegQueryValueExA(HKEY_CLASSES_ROOT, szKey, NULL, &dwType,
					  szDDEExecValue, &lTmp)
			    == ERROR_SUCCESS)
			{
...
HRESULT MyOpenAsDialog(HWND hwnd, POPENASINFO poainfo)
{
	...
	if ((RegQueryValueExA(HKEY_CLASSES_ROOT, oad.lpszExt, NULL, &dwType, oad.szTypeName, &lTmp) == ERROR_SUCCESS)
