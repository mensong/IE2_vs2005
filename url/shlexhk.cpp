...
HRESULT STDMETHODCALLTYPE InternetShortcut::Execute(LPSHELLEXECUTEINFOA pei)
{
   HRESULT hr;
   ...
   if (! pei->lpVerb ||
       ! lstrcmpiA(pei->lpVerb, s_cszOpenVerb))
   {
      if (pei->lpFile)
      {
         PSTR pszTranslatedURL;

         hr = TranslateURLA(pei->lpFile, TRANSLATEURL_FL_GUESS_PROTOCOL,
                           &pszTranslatedURL);
