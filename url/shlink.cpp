                                                    ...
                                                    DWORD dwFlags)
{
   ...
   ASSERT(IS_VALID_STRUCT_PTR(pwfd, CWIN32_FIND_DATA));
   ...
   if (m_pszURL)
   {
      MyLStrCpyN(pszFile, m_pszURL, ncFileBufLen);
                                                         ...
                                                         PCSTR pcszDescription)
{
   ...
   bDifferent = (! m_pszFile ||
                 lstrcmpA(pcszDescription, m_pszFile) != 0);
                                                      ...
                                                      int ncDesciptionBufLen)
{
   ...
   if (m_pszFile)
      MyLStrCpyN(pszDescription, m_pszFile, ncDesciptionBufLen);
   else
   {
                                                   ...
                                                   PCSTR pcszWorkingDirectory)
{
   ...
   if (pcszWorkingDirectory)
   {
      ...
      if (GetFullPathNameA(pcszWorkingDirectory, sizeof(rgchNewPath),
   ...
   if (hr == S_OK)
   {
      bChanged = ! ((! pcszWorkingDirectory && ! m_pszWorkingDirectory) ||
                    (pcszWorkingDirectory && m_pszWorkingDirectory &&
                     ! lstrcmpA(pcszWorkingDirectory, m_pszWorkingDirectory)));

      if (bChanged && pcszWorkingDirectory)
      {
         // (+ 1) for null terminator.

         pszNewWorkingDirectory = new(char[lstrlenA(pcszWorkingDirectory) + 1]);

         if (pszNewWorkingDirectory)
            lstrcpyA(pszNewWorkingDirectory, pcszWorkingDirectory);
         else
            hr = E_OUTOFMEMORY;
      }
   }
                                                ...
                                                int ncbWorkingDirectoryBufLen)
{
   ...
   if (m_pszWorkingDirectory)
   {
      MyLStrCpyN(pszWorkingDirectory, m_pszWorkingDirectory,
   ...
   if (hr == S_OK)
   {
      ...
      if (SUCCEEDED(hr))
      {
         ...
         bChanged = ((! bOldMeat && bNewMeat) ||
                     (bOldMeat && ! bNewMeat) ||
                     (bOldMeat && bNewMeat &&
                      (lstrcmpA(rgchOldPath, rgchNewPath) != 0 ||
                       niIcon != niOldIcon)));
         ...
         if (bChanged && bNewMeat)
         {
            ...
            pszNewIconFile = new(char[lstrlenA(rgchNewPath) + 1]);

            if (pszNewIconFile)
            {
               lstrcpyA(pszNewIconFile, rgchNewPath);
               niNewIcon = niIcon;
                                                         ...
                                                         PINT pniIcon)
{
   ...
   if (m_pszIconFile)
   {
      MyLStrCpyN(pszIconFile, m_pszIconFile, ncbIconFileBufLen);
      *pniIcon = m_niIcon;
