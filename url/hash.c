...
#endif
#include "project.h"
#include "../security/ssl/code/guts.h"
...
LPCTSTR NEAR PASCAL LookupItemInHashTable(PHASHTABLE pht, LPCSTR pszName, int iOp)
{
    ...
    case ADDHASHITEM:
        ...
        else
        {
            ...
            phi = (PHASHITEM)malloc(sizeof(HASHITEM) + cbName + pht->wcbExtra);
...
LPCTSTR WINAPI FindHashItem(PHASHTABLE pht, LPCSTR lpszStr)
{
...
LPCTSTR WINAPI AddHashItem(PHASHTABLE pht, LPCSTR lpszStr)
{
...
LPCTSTR WINAPI DeleteHashItem(PHASHTABLE pht, LPCSTR lpszStr)
{
...
void CALLBACK _DeleteHashItem(LPCSTR sz, UINT usage)
{
    free(HIFROMSZ(sz));
} 
