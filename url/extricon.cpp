                                    ...
                                    UINT ucbIconFileBufLen, PINT pniIcon)
{
   ...
   if ((UINT)lstrlenA(pszIconEntry) < ucbIconFileBufLen)
   {
      lstrcpyA(pszIconFile, pszIconEntry);
      hr = S_OK;
                                               ...
                                               PINT pniIcon)
{
   ...
   {
      lstrcpyA(pszIconFile, s_cszGenericURLIconFile);
...
PUBLIC_CODE BOOL AnyMeat(PCSTR pcsz)
{
   ...
   return(pcsz ? strspn(pcsz, g_cszWhiteSpace) < (UINT)lstrlenA(pcsz) : FALSE);
}
...
PUBLIC_CODE HRESULT CopyURLProtocol(PCSTR pcszURL, PSTR *ppszProtocol)
{
   ...
   *ppszProtocol = NULL;
   ...
   {
      // (+ 1) for null terminator.
      *ppszProtocol = new(char[pu.ucbProtocolLen + 1]);

      if (*ppszProtocol)
      {
         // (+ 1) for null terminator.
         MyLStrCpyN(*ppszProtocol, pu.pcszProtocol, pu.ucbProtocolLen + 1);
         ASSERT((UINT)lstrlen(*ppszProtocol) == pu.ucbProtocolLen);
      }
      else
         hr = E_OUTOFMEMORY;
   }
...
PUBLIC_CODE HRESULT CopyURLSuffix(PCSTR pcszURL, PSTR *ppszSuffix)
{
   ...
   *ppszSuffix = NULL;
   ...
   {
      // (+ 1) for null terminator.
      *ppszSuffix = new(char[pu.ucbSuffixLen + 1]);

      if (*ppszSuffix)
      {
         // (+ 1) for null terminator.
         MyLStrCpyN(*ppszSuffix, pu.pcszSuffix, pu.ucbSuffixLen + 1);
         ASSERT((UINT)lstrlen(*ppszSuffix) == pu.ucbSuffixLen);
                                   ...
                                   PSTR *ppszKey)
{
   ...
   // (+ 1) for null terminator.
   ulcbKeyLen = lstrlenA(pcszProtocol) + 1 + lstrlenA(pcszSubKey) + 1;

   *ppszKey = new(char[ulcbKeyLen]);

   if (*ppszKey)
   {
      lstrcpyA(*ppszKey, pcszProtocol);
      CatPath(*ppszKey, pcszSubKey);
   ...
   if (IS_FLAG_CLEAR(uInFlags, GIL_OPENICON))
   {
      ...
      if (hr != S_OK)
      {
         ...
         if (hr == S_OK)
         {
            ...
            if (hr == S_OK)
            {
               if ((UINT)lstrlenA(rgchFullPath) < ucbIconFileBufLen)
                  lstrcpyA(pszIconFile, rgchFullPath);
               else
                  hr = E_FAIL;
            }
         }
